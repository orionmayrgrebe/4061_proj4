...
int main(int argc, char *argv[]) {
    ...
    // create log file
    createLogFile();
    int final_file = open("log/log_client.txt", O_WRONLY | O_CREAT, 0644);
    ...
    if(pid == 0) {
      ...
      // check response
      printf("[%d] GET_AZLIST: %d %d %d %d %d %d %d %d %d %d %d %d", long_response[2], long_response[3], long_response[4], long_response[5], long_response[6], long_response[7], long_response[8], long_response[9], long_response[10], long_response[11], long_response[12], long_response[13], long_response[14]);
      ...
      // we are in the master client and need to wait
      wait(NULL);
    }
    // close files
    fclose(logfp);
